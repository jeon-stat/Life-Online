...
const STATE_PROFILES = {
  [ENERGY_STATES.LOW_ENERGY]: {
    ...
    weightBias: {
      ...
      [ACTION_KEYS.sleepyIdle]: 12,
      [ACTION_KEYS.lookingDown]: 10,
  ...
  [ENERGY_STATES.HIGH_ENERGY]: {
    ...
    weightBias: {
      ...
      [ACTION_KEYS.dashStart]: 10,
      [ACTION_KEYS.excitedIdle]: 6,
    },
    clipSpeedScale: 1.08,
...
export function buildBehaviorProfile({ steps = 0, history = [], goal = DEFAULT_STEP_GOAL, overrides = {} } = {}) {
  ...
  const rawMainActions = buildActionPool(energyProfile.mainActionKeys, ACTION_TYPES.MAIN, {
    stateProfile: energyProfile,
    longTermProfile,
    timing,
    overrides,
  });
  const rawTransitionActions = buildActionPool(energyProfile.transitionActionKeys, ACTION_TYPES.TRANSITION, {
    stateProfile: energyProfile,
    longTermProfile,
    timing,
    overrides,
  });
  const mainActions = rawMainActions.filter((action) => action.playable);
  const transitionActions = rawTransitionActions.filter((action) => action.playable);
  const allActions = [...rawMainActions, ...rawTransitionActions];
